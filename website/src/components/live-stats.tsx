...
import { fetchOnchainStats, type OnchainStats } from '@/lib/onchain-stats';
 ...
 * streaming live now (ticks up every second). Figures are read from our mainnet
 * caveat enforcer events (see lib/onchain-stats).
 */
...
export function LiveStats() {
  ...
  // Read the enforcer events, then re-poll so newly-charged delegations and fresh
  // claims show up without a reload (constant auto-discovery — no hash list).
  useEffect(() => {
    let cancelled = false;
    ...
    const load = async () => {
      ...
      const current = ctrl;
      const timer = setTimeout(() => current.abort(), 25_000);
      try {
        const s = await fetchOnchainStats(current.signal);
        if (!cancelled) {
          setStats(s);
          fetchedRef.current = performance.now();
        }
      } catch {
        // keep the previous values; surface as loaded so it doesn't hang on '—'
      } finally {
        clearTimeout(timer);
  ...
  // Animation clock — drives the live streaming tick from the fetch instant.
  useEffect(() => {
    if (reduced) return;
    let raf = 0;
    const tick = (t: number) => {
  ...
  const elapsed = fetchedRef.current !== null ? Math.max(0, (now - fetchedRef.current) / 1000) : 0;
  const settled = stats?.settled ?? 0;
  const streaming = (stats?.streamingBase ?? 0) + (stats?.streamingRate ?? 0) * elapsed;

  return (
    <div className="relative aspect-square w-full max-w-[460px]">
      {/* dark radial backdrop so the figures stay legible over the hero video */}
      <div
        className="absolute inset-0 rounded-full"
        style={{ background: 'radial-gradient(circle, rgba(10,10,11,0.78) 42%, rgba(10,10,11,0) 72%)' }}
      />
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center px-12 font-mono">
        ...
        <div className="text-center">
          ...
          <div className="mt-1 text-[10px] tracking-[0.25em] text-[color:var(--color-faint)]">
            USDC / s{loaded ? ` · ${stats?.delegations ?? 0} settled` : ''}
          </div>
        </div>
      </div>
    </div>
